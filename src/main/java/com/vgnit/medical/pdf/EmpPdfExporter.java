...
public class EmpPdfExporter {
    ...
    private List<Emp> listEmps;

    public EmpPdfExporter(List<Emp> listEmps) {
    ...
    private void writeTableData(PdfPTable table) {
        for (Emp row : listEmps) {
            table.addCell(String.valueOf(row.getId()));
            table.addCell(row.getTypeEmp());
    ...
    public void export(HttpServletResponse response) throws DocumentException, IOException {
        ...
        document.open();
        document.add(new Paragraph("LIST OF EMPLOYEES"));
